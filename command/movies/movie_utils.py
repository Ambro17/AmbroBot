...
def get_movie_info(movie):
    ...
    r = requests.get(f"https://api.themoviedb.org/3/movie/{movie['id']}", params=params)
    data = r.json()
...
def get_trackers():
    ...
    return ''.join(f'&tr={tracker}' for tracker in trackers)
...
def get_magnet_link(title, imdb_id):
    ...
    if jash:
        return MAGNET.format(
            torrent_hash=jash, movie_name=encode_for_url(title), trackers=get_trackers()
        )
    return None
